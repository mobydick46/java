...
<script type="text/javascript">
	...
	*/
	...
	function addCalculationObject(nodeType){
        ...
        if(0 != rowSources.length && "" == rowSources[rowSources.length -1].objectCode){
        	$.messager.alert("系统提示", "请首先完善上一条插入的信息，确保输入的编码未被使用！");
			return;
		}
	...
	/*添功能类型-决策计划*/
	function addDecisionPlan(nodeID){
		var rowSources = $('#dg').datagrid('getRows');
		var rowSource = undefined;
        if(0 != rowSources.length && "" == rowSources[rowSources.length -1].planName){
        	$.messager.alert("系统提示", "请首先完善上一条插入的信息，并确保添加的项目为模型方法-结果输出的选项！");
			return;
		}
        if(0 != rowSources.length){
			rowSource = rowSources[rowSources.length -1];
		}
		$.post("${pageContext.request.contextPath}/decisionplan/getmaxnumber.do",
				function(result) {
					if (result.success) {
						...
						var url = "${pageContext.request.contextPath}/decisionplan/add2.do";
						var functionId = $('#tt').tree('getParent', $('#tt').tree('find', nodeID).target).attributes.functionId;
						var timeID = times[nodeTypeid].timeID;
						var map = {number:number,timeID:timeID,functionId:functionId};
						if(1 != rowSources.length){
							url = "${pageContext.request.contextPath}/decisionplan/add.do";
							map = {number:number,numberSource:rowSource.number};
						}
						$.post(url,map,
								function(result) {
									if (null != result) {
	...
	function cancelData(){
		...
		if(0 == rowsData.length){
			$.messager.alert("系统提示", "请选择所要删除的数据！");
			return;
		}
		for(var i=0;i<rowsData.length;i++){
			if(4 == flag && 1 != nodes.id && 1 == nodes.id%8){
				strIds.push(rowsData[i].number);
			}else{
				strIds.push(rowsData[i].id);
			}
			var ids = strIds.join(",");
	...
	function OK(){
		...
		if(1 >= rows.length){
			$.messager.alert("系统提示", "只有<font color=red>"+rows.length+"</font>条数据，无法操作！");
			return;
		}
		var strIds = [];
	...
	*/
		...
		switch(flag){
		    case 1:
		    	switch(nodeID){
		    	    ...
		    	    case 3:
		    	       toolbar = [{iconCls: 'icon-add',text:'添加',handler: function(){addData()}},'-',{iconCls: 'icon-cancel',text:'删除',handler: function(){cancelData()}
		    	        },'-'];
		    	       columns=[[
		    	  	               {field:'id',title:'',width:80,hidden:'true'},
		    ...
		    case 2:if(11 == nodeID || 12 == nodeID){
		           ...
		           toolbar = [{iconCls: 'icon-edit',text:'全选',handler: function(){updateCalculationObject(1)}},'-',
		                      {iconCls: 'icon-edit',text:'反选',handler: function(){updateCalculationObject(2)}},'-',
		    	              {iconCls: 'icon-edit',text:'清除',handler: function(){updateCalculationObject(3)}},'-',
		    	              {iconCls: 'icon-edit',text:'刷新',handler: function(){alert('help')}
		    	              }];
		           if(!(0 == parentObject.length || "" == parentObject[0].parentCode)){
		        	   toolbar.push('                ');
		    ...
		    case 3:if(1==nodeID){
    	             columns=[[
    		           ...
    		           toolbar=[{iconCls: 'icon-cancel',text:'删除',handler: function(){cancelData()}},'-',
    		                    {iconCls: 'icon-edit',text:'提取',handler: function(){alert(1)}},'-',
    		                    {iconCls: 'icon-edit',text:'确定',handler: function(){getNewTree()}}
    		                    ];}
				    else if(2!=nodeID%5){
				    	columns=[[
	  	    	  	               {field:'id',title:'',width:80,hidden:'true'},
		...
		$('#dg').datagrid({
		   ...
		   loadFilter : function(result) {
			...
			onRowContextMenu : function(e, rowIndex, rowData){
				...
				if((1 == flag && (3 == node.id || 4 == node.id || 5 == node.id)) 
				   || (4 == flag && 1 != node.id && (3 == node.id%8 || 5==node.id%8 || 1 == node.id%8))){
					rowIndex1 = "";
					rowData1 = "";
					rowIndex2 = rowIndex;
					rowData2 = rowData;
				...
				function(result) {
					if (result.success) {
						if(result.data){
							...
							$.post("${pageContext.request.contextPath}/modelmethod/addInputOutput.do",{combCalculation:"√"},
							function(result) {
								if (result) {
								} else {
									$.messager.alert(result.errormsg, result.errorinfo);
	...
	function download(){
		...
		$.post(url,{path:path},function(result) {
			if (result.success){
				$.messager.alert("系统提示", "保存成功,保存路径为<font color=red>" + result.path +"</font>！");
			} else {
				$.messager.alert("系统提示", "保存失败,请检查路径是否正确：<font color=red>" + result.path +"</font>！");
			}
		}, 
"json");
	}
